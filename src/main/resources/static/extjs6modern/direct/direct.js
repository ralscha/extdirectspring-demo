	...
	launch: function() {
		function doEcho(field) {
			touchTestAction.doEcho(field.getValue(), function(result, event) {
				...
				var content = Ext.String.format('<b>Successful call to {0}.{1} with response:</b><pre>{2}</pre>', transaction.action, transaction.method, Ext.encode(result));
		...
		function doMultiply(field) {
			touchTestAction.multiply(field.getValue(), function(result, event) {
				...
				content = Ext.String.format('<b>Successful call to {0}.{1} with response:</b><pre>{2}</pre>', transaction.action, transaction.method, Ext.encode(result));
				updateMain(content);
				field.reset();
			});
		}
